...
set "META_JAVA_SRC=%CMETA_ROOT%\c3dclasses_java\src\main\java"
for %%I in ("%META_JAVA_SRC%") do set "META_JAVA_SRC=%%~fI"
...
if /I not "%SRC_EXT%"==".java" (
	echo [SKIP] Non-Java file change.
	goto update
)

if not exist "%SRC_FILE%" (
	echo [SKIP] Source file does not exist ^(likely delete/rename source^): %SRC_FILE%
	goto update
)
...
if /I "%REL_PATH%"=="%SRC_FILE%" (
	echo [SKIP] File not under core Java source root: %CORE_ROOT%
	goto update
)

set "DST_FILE=%META_JAVA_SRC%\%REL_PATH%"
